...
def send_message(message):
    ...
    data = {
        "content": f"{current_time} EST: {message}"
    }
...
def change_font_size(size):
    time_label.config(font=("Helvetica", size))
    commands_label.config(font=("Helvetica", size))
    font_size_label.config(font=("Helvetica", size))
    font_color_label.config(font=("Helvetica", size))
    font_size_dropdown.config(font=("Helvetica", size))
    font_color_dropdown.config(font=("Helvetica", size))
    set_webhook_button.config(font=("Helvetica", size))
    fullscreen_button.config(font=("Helvetica", size))
    custom_message_label.config(font=("Helvetica", size))
    for button in predefined_message_buttons:
        button.config(font=("Helvetica", size))
    custom_message_entry.config(font=("Helvetica", size))
    send_custom_button.config(font=("Helvetica", size))
...
def change_font_color(color):
    ...
    commands_label.config(fg=color)
    font_size_label.config(fg=color)
    font_color_label.config(fg=color)
    font_size_dropdown.config(fg=color)
    font_color_dropdown.config(fg=color)
    set_webhook_button.config(fg=color)
    fullscreen_button.config(fg=color)
    custom_message_label.config(fg=color)
    for button in predefined_message_buttons:
        button.config(fg=color)
    custom_message_entry.config(fg=color)
    send_custom_button.config(fg=color)
...
def toggle_fullscreen():
    if window.attributes("-fullscreen"):
        ...
        window.geometry(f"{window.normal_width}x{window.normal_height}+0+0")  # Reset window geometry to its original size and position
    else:
        window.normal_width = window.winfo_width()
        window.normal_height = window.winfo_height()
        window.attributes("-fullscreen", True)
        change_element_size(16)  # Adjust element size for fullscreen mode

# Function to change element size based on window state
def change_element_size(size):
    change_font_size(size)
    change_font_color(settings["font_color"])
...
# Fullscreen button
fullscreen_button = tk.Button(window, text="Fullscreen", command=toggle_fullscreen, font=("Helvetica", settings["font_size"]))
